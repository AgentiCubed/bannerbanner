...
export function RealWorldTester() {
  ...
  const exportTestReport = () => {
    ...
    const totalTests = Object.values(testResults || {}).reduce((sum, r) => sum + r.attemptCount, 0);
    const testedSites = Object.keys(testResults || {}).length;
    
    const workingSites = testSites.filter((s) => getStatusFromResult(s.testResult) === 'working');
    const partialSites = testSites.filter((s) => getStatusFromResult(s.testResult) === 'partial');
    const failedSites = testSites.filter((s) => getStatusFromResult(s.testResult) === 'failed');
    const untestedSites = testSites.filter((s) => getStatusFromResult(s.testResult) === 'untested');

    const report = `# BannerBanner Test Report
...
- **Sites Tested**: ${testedSites} / ${allSites.length}

## Results Breakdown

- ✅ **Working**: ${workingSites.length} sites
- ⚠️ **Partial**: ${partialSites.length} sites
- ❌ **Failed**: ${failedSites.length} sites
- ⏸️ **Untested**: ${untestedSites.length} sites
...
${Object.entries(testResults || {})
  ...
  .map(([url, result]) => {
    ...
    const statusEmoji = result.status === 'working' ? '✅' : result.status === 'failed' ? '❌' : result.status === 'partial' ? '⚠️' : '⏸️';
    const siteSuccessRate = result.attemptCount > 0 ? Math.round((result.successCount / result.attemptCount) * 100) : 0;
    
    return `### ${statusEmoji} ${site.name}

**URL**: ${site.url}  
**Banner Type**: ${site.bannerType}  
**Status**: ${result.status.toUpperCase()}  
**Success Rate**: ${siteSuccessRate}% (${result.successCount}/${result.attemptCount} attempts)

**Detection Details**:
- Banner Detected: ${result.bannerDetected ? '✓ Yes' : '✗ No'}
- Banner Closed: ${result.bannerClosed ? '✓ Yes' : '✗ No'}
- Cookies Verified: ${result.cookiesVerified ? '✓ Yes' : '○ No'}

**Last Tested**: ${new Date(result.lastAttemptTimestamp || result.timestamp).toLocaleString()}
...
${workingSites.map(s => `- ✅ **${s.name}** (${s.bannerType})`).join('\n') || 'None'}

## Failed Sites (${failedSites.length})

${failedSites.map(s => `- ❌ **${s.name}** (${s.bannerType})`).join('\n') || 'None'}

## Recommendations

${failedSites.length > 0 ? `
### Failed Sites Require Attention
The following sites need pattern improvements or manual testing:
${failedSites.map(s => `- ${s.name}: ${s.url}`).join('\n')}
` : ''}

${partialSites.length > 0 ? `
### Partial Success Sites
These sites work partially and may need refinement:
${partialSites.map(s => `- ${s.name}: ${s.url}`).join('\n')}
` : ''}

${untestedSites.length > 0 ? `
### Untested Sites
Complete testing on these sites:
${untestedSites.map(s => `- ${s.name}: ${s.url}`).join('\n')}
` : ''}

${successRate >= 90 ? '✅ **Excellent coverage!** BannerBanner is performing very well.' : 
  successRate >= 70 ? '⚠️ **Good coverage** with room for improvement on failed patterns.' : 
  '❌ **Needs improvement** - Focus on debugging failed patterns and improving detection.'}

---

*Generated by BannerBanner Testing Framework*
`;
    ...
    toast.success('Test report exported successfully!', {
      description: 'Check your downloads folder for the markdown file',
    });
  };
  ...
  const copyReportToClipboard = () => {
    ...
    const quickReport = `BannerBanner Test Results (${new Date().toLocaleDateString()})

Success Rate: ${successRate}% | Tests: ${totalTests} | Sites: ${testedSites}/${allSites.length}
✅ Working: ${workingSites.length} | ❌ Failed: ${failedSites.length}

${workingSites.length > 0 ? `Working Sites:\n${workingSites.map(s => `✅ ${s.name} (${s.bannerType})`).join('\n')}` : ''}
${failedSites.length > 0 ? `\nFailed Sites:\n${failedSites.map(s => `❌ ${s.name} (${s.bannerType})`).join('\n')}` : ''}`;

    navigator.clipboard.writeText(quickReport).then(() => {
