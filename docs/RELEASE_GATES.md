...
- [ ] **PB-11: Real-site acceptance**
  ...
  - Evidence: Partial preparation only. Matrix has 29 candidate rows covering 20 concrete distinct origins (plus two test-time placeholders), protocol, CMP fixture mode matrix, automated permission/sensitive evidence links, and Run R1 environment/provenance (`docs/REAL_SITE_TEST_MATRIX.md`, `docs/REAL_SITE_TEST_PROTOCOL.md`, `docs/RELEASE_PROVENANCE.md`). Passing rows must link committed sanitized evidence or a durable external artifact. **Live site rows remain `Not run`** — agent/CI hosts cannot resolve external DNS; a human desktop Chrome run of the protocol is still required before this gate can close.
