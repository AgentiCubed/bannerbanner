...
- [ ] **PB-02: Complete revocation**
  ...
  - Evidence: Automated: `extension/test/unit/registry.test.mjs` (revocation/unregistration planning, update cannot broaden), `extension/test/unit/background.test.mjs` (an update removes a stale revoked registration and does not restore the origin), and the BB_STOP test in `extension/test/browser/specs/pipeline.spec.mjs`. Pending: manual revoke, post-revoke restart, and post-revoke update rows in the matrix.
...
- [ ] **PB-05: Sensitive workflows remain untouched**
  ...
  - Evidence: Automated: `extension/test/unit/classify.test.mjs` plus one fixture regression per protected class in `extension/test/browser/specs/pipeline.spec.mjs` (matrix sensitive section marked Pass for fixtures). Pending: the two required Run R2 real-flow spot checks.
...
- [ ] **PB-07: Shipped settings control shipped behavior**
  ...
  - Evidence: Automated: `extension/test/unit/settings-schema.test.mjs` (versioned schema, migration, corruption recovery), `extension/test/unit/background.test.mjs` (current settings and an authorized grant survive update), and the options-UI persistence test in `boundary.spec.mjs`. Pending: manual restart/update verification.
...
- [ ] **PB-09: Complete extension package**
  ...
  - Evidence: Automated: `scripts/validate-package.mjs` (missing files fatal, strict inventory) run inside `extension/build-extension.sh` and CI; icons committed and reproducible. The build now emits a deterministic, portable-checksum archive; two local candidate builds at `6f8255b` matched. Pending: fresh-profile manual load check.
...
- [ ] **PB-10: Automated release checks**
  ...
  - Evidence: Automated: `.github/workflows/extension-ci.yml` runs syntax checks, unit tests, icon reproducibility, build + package validation, and both browser suites on every pull request; both release workflows also support manual dispatch and `v*` tag pushes. Local commands are documented in `extension/README.md`. `.github/workflows/lint.yml` runs repo-wide eslint (flat config in `eslint.config.js` covering `src/`, `extension/`, `scripts/`, and tests). Candidate `6f8255b` passes locally (73 unit, 33 browser, lint with 0 errors); exact-SHA GitHub workflow URLs are still pending.
...
- [ ] **PB-11: Real-site acceptance**
  ...
  - Evidence: Partial preparation only. Matrix has 29 candidate rows covering 20 distinct candidate sites once its two placeholders are selected separately, an exact-candidate preflight table, complete grant/revoke/restart/update and storage/package audit rows, CMP fixture coverage, and Run R1 provenance (`docs/REAL_SITE_TEST_MATRIX.md`, `docs/REAL_SITE_TEST_PROTOCOL.md`, `docs/RELEASE_PROVENANCE.md`). Candidate `6f8255b` and deterministic archive hash are recorded, but exact-SHA CI links are pending. **Live site rows remain `Not run`** — a human desktop Chrome run is still required before this gate can close.
...
- [ ] **WS-01: Single purpose and minimum permissions**
  ...
  - Evidence: Draft Store summary/description and permission justifications in `extension/CHROME_WEB_STORE_SUBMISSION.md` match `manifest.json` (`storage`, `activeTab`, `scripting`, optional hosts) and the MVP contract. Final dashboard paste and dated confirmation remain pending.
...
- [ ] **WS-02: Accurate privacy disclosures**
  ...
  - Evidence: Partial: `extension/PRIVACY_POLICY.md` matches the exact storage schema (`bb:settings`, `bb:authorizedOrigins`, `bb:stats`) with the empty-sync assertion tested in `boundary.spec.mjs`. Pending: a public policy URL (the private repository URL returned 404 signed out) and Web Store dashboard answers.
...
- [ ] **WS-03: Listing and asset readiness**
  ...
  - Evidence: Icons and v0.1-aligned listing copy exist. The 440×280/1400×560 asset helper was stripped of alpha training/theme claims and no longer loads remote JSZip; the submission checklist now enforces real UI screenshots and full-size privacy/claim review. Pending: exported/reviewed promo tile, real popup/options screenshot, public support URL, and dashboard upload evidence.
...
- [ ] **WS-04: Release artifact provenance**
  ...
  - Evidence: Build procedure + strict inventory enforced by `extension/build-extension.sh` / `scripts/validate-package.mjs`; archives now normalize timestamps/order/metadata and emit portable checksums. `docs/RELEASE_PROVENANCE.md` records a twice-reproduced pre-R2 candidate hash. Final evidence still requires all PB gates, a release tag, green tag workflows, and the exact uploaded archive hash.
