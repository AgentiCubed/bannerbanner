...
8. **Capture evidence**: screenshot of the settled page + a DevTools shot of
   ...
   no personal data in shots. Commit sanitized evidence or link it to a durable
   external artifact. Exclude captures that cannot be safely sanitized; they
...
- If every claimed CMP passed both its claimed modes with zero destructive
  false positives across 20–30 distinct site origins (duplicate mode rows for
  one origin count once), update `PB-11` in
  `docs/RELEASE_GATES.md` with a link to the matrix section and the evidence
  committed in the repository or stored in a durable external artifact, and
